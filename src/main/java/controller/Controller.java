...
public class Controller {
	...
	public ResponseEntity<APIResponse<List<CiaDeSeguros>>> buscarTodos() {
		APIResponse<List<CiaDeSeguros>> response = new APIResponse<List<CiaDeSeguros>>(200, null,
				service.buscarTodos());
		return ResponseEntity.status(HttpStatus.OK).body(response);
	}

	@GetMapping("/CiaDeSeguros/{id}") // consultar
	public ResponseEntity<APIResponse<CiaDeSeguros>> buscarporId(@PathVariable("id") Integer id) { // filtrado por
																									// segmento
																									// específico
		CiaDeSeguros companiaPorId = service.buscarPorId(id);
		if (this.existe(id)) {
			APIResponse<CiaDeSeguros> response = new APIResponse<CiaDeSeguros>(HttpStatus.OK.value(), null,
					companiaPorId);
			return ResponseEntity.status(HttpStatus.OK).body(response);
		} else {
			List<String> messages = new ArrayList<>();
			messages.add("No encontrada Compañía con ID: " + id.toString());
			APIResponse<CiaDeSeguros> response = new APIResponse<CiaDeSeguros>(HttpStatus.BAD_REQUEST.value(), messages,
					companiaPorId);
	...
	public ResponseEntity<APIResponse<CiaDeSeguros>> crearCiaDeSeguros(@RequestBody CiaDeSeguros compania) { // inyecta
																												// un
																												// objeto
																												// nuevo
																												// a la
		...
		if (this.existe(compania.getId())) {
			...
			APIResponse<CiaDeSeguros> response = new APIResponse<CiaDeSeguros>(HttpStatus.CREATED.value(), null,
					compania);
	...
	public ResponseEntity<APIResponse<?>> handleConstraintViolationException(ConstraintViolationException ex) {
		...
		APIResponse<CiaDeSeguros> response = new APIResponse<CiaDeSeguros>(HttpStatus.BAD_REQUEST.value(), errors,
				null);
